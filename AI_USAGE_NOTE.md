...
* **Groq LLM API (`groq/compound-mini`)**: Used in the backend for natural language intent interpretation (`/analyze`) and structured research synthesis (`/explain`).
* **ChatGPT & Gemini**: Used during initial system design to brainstorm parameter extraction edge cases, financial methodology risks, and look-ahead bias prevention.
* **Cursor / Antigravity Agent**: Used for rapid full-stack scaffolding, Pydantic model validation alignment, and Next.js component creation.
